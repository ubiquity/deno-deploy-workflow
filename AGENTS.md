...
- For new-platform Deno 2 migrations, use the reusable workflow with `deploy_platform: deno2`; it creates apps with `deno deploy create`, syncs env with `deno deploy env load`, deploys with `deno deploy`, and emits `.deno.net` URLs. Deploy Classic (`deployctl`, `.deno.dev`) is legacy during the cutover.
- Prefer Deno-native install/build commands for Deno 2 migrations (`deno install`, `deno task build`). Keep Bun only where a repo genuinely needs it.
...
- New Deno Deploy app URLs use `https://<app>.<org>.deno.net`; do not treat a `.deno.dev` URL as proof of a Deno 2 migration.
