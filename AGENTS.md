...
- Deno Deploy project names must end with `-ubq-fi` to match the router’s `<sub>-ubq-fi.deno.dev` mapping. Preview mode controls naming: `branch` (default) creates per-branch projects `<slug>-<base>-ubq-fi` with router URLs `<slug>-<base>.ubq.fi`; `shared` uses legacy `p-<base>-ubq-fi` with `preview-<base>.ubq.fi`. All names are clamped to 26 chars with truncation + short hash. Per-branch projects are cleaned up on branch deletion via `deno-deploy-cleanup.yml`.
